...
fn copy_finite_with_sign_round<L: Limb>(
    ...
    if !msb && !lsb_any {
        dst.mantissa.copy_from_slice(&src.mantissa[limb_diff..]);
        Approx::Exact
    } else {
            ...
            || (rnd == RoundKnownSign::ToNearest && msb && (lsb_any || mantissa_odd))
        {
            ...
            (dst.mantissa[0], carry) = (src_m[0] & !(ulp.wrapping_sub(one))).overflowing_add(ulp);

            for (dst, &src) in core::iter::zip(&mut dst.mantissa[1..], &src_m[1..]) {
            ...
            // round to zero
            dst.mantissa.copy_from_slice(&src.mantissa[limb_diff..]);
            dst.mantissa[0] &= !(ulp.wrapping_sub(one));
...
pub fn copy_with_sign<L: Limb>(
    ...
    let approx = if dst.precision_bits() >= src.precision_bits() {
        dst.mantissa[..src.mantissa.len()].copy_from_slice(&src.mantissa);
        dst.mantissa[src.mantissa.len()..].fill(L::ZERO);
        Approx::Exact
    } else {
...
mod tests {
    ...
    fn test_widening_copy() {
        ...
        copy(&mut dst, &src, Round::ToNearest);
